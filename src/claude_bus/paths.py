"""Database path resolver for raven.

Resolution order:

1. Explicit ``db_path`` argument (always wins).
2. ``RAVEN_DB`` environment variable.
3. ``./raven.db`` in the current working directory.
"""
...
def resolve_db_path(db_path: str | os.PathLike[str] | None = None) -> Path:
    """Return the path to the raven SQLite file.
